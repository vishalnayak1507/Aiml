...
class RNN:
    ...
    def forward(self, X, a_prev):
        ...
        for i in range(len(self.X)):
            ...
            a[i] = np.tanh(np.dot(self.Wax, x[i]) + np.dot(self.Waa, a[i - 1]) + self.ba)
    ...
    def loss(self, y_preds, targets):
        ...
        #loss = -np.sum(-np.log(y_preds[t][targets[t]]) for t in range(len(self.X)))
        #temp1 = np.array(list(y_preds.values()))[:,:,0]
        #one_hot_targets = np.eye(temp1.shape[1])[targets]
        #temp = one_hot_targets*np.log(temp1)
        #loss = np.sum(temp)
